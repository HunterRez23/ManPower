...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (ex.rows.length) {
      return res.json({ ok:true, msg:'Ya te habías postulado' });
    }
    ...
    return res.json({ ok:true, data: ins.rows[0] });
  } catch (e) {
    console.error(e);
...
router.put('/:id_postulacion', async (req, res) => {
  ...
  const { estado } = req.body; // 'aceptada' | 'rechazada' | 'recibida'
  if (!estado) return res.status(400).json({ ok:false, msg:'estado requerido' });
  try {
    ...
    return res.json({ ok:true, data: up.rows[0] });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ ok:false, error: e.message });
  }
